...
export const useStreamViewer = ({
  ...
  const waitForWebSocketReady = useCallback(
    async (session: Session, maxWaitTime = 15000): Promise<boolean> => {
      return new Promise(resolve => {
        ...
        const checkWebSocketState = () => {
          ...
          try {
            ...
            console.warn(
              '[useStreamViewer] WebSocket 상태 확인 중 에러:',
              error,
            );
          }
    ...
    try {
      ...
      return new Promise(resolve => {
        ...
        pc.onicecandidate = event => {
          if (event.candidate) {
            console.log(
              '✅ [useStreamViewer] STUN 서버 연결 성공 - ICE Candidate 수집됨',
            );
        ...
        setTimeout(() => {
          if (!candidateFound) {
            console.warn(
              '❌ [useStreamViewer] STUN 서버 연결 실패 - ICE Candidate 수집 안됨',
            );
            pc.close();
            resolve(false);
          }
   ...
   * 네트워크 품질을 측정하는 함수
   */
  const measureNetworkQuality = useCallback(async (): Promise<
    'good' | 'medium' | 'poor'
  > => {
    try {
      ...
      console.log(
        `📊 [useStreamViewer] 네트워크 품질 측정: ${latency.toFixed(2)}ms`,
      );
      ...
      console.warn(
        '📊 [useStreamViewer] 네트워크 품질 측정 실패, 기본값 사용:',
        error,
      );
  ...
  const connectToStream = useCallback(
    async (token: string, videoElementId: string) => {
      ...
      if (connectingTokenRef.current === token) {
        console.warn(
          '⚠️ [useStreamViewer] 동일한 토큰으로 연결 중 - 중복 시도 차단',
        );
        return;
      }
      ...
      if (
        ...
        console.warn(
          '⚠️ [useStreamViewer] 이미 연결된 상태 - 중복 연결 시도 차단:',
      ...
      try {
        ...
        if (!isStunTurnAccessible) {
          console.warn(
            '⚠️ [useStreamViewer] STUN/TURN 서버 접근 불가 - WebRTC 연결 제한적',
          );
          ...
          // 사용자에게 즉시 안내
          toast.warning(
            '네트워크 제한이 감지되었습니다. 개인 네트워크 사용을 권장합니다.',
          );
        }
          ...
          if (stream && (stream as any).connection && (stream as any).connection.connection) {
            ...
            rtcPeerConnection.addEventListener(
              ...
              () => {
                ...
                console.log(
                  `🧊 [useStreamViewer] ICE 연결 상태 변경 (${iceConnectionStateChangeCountRef.current}/${maxIceConnectionChanges}):`,
                );
                console.log(
                  `   이전: ${previousState} → 현재: ${iceConnectionState}`,
                );
            ...
            rtcPeerConnection.addEventListener(
              ...
              () => {
                console.log(
                  '🔍 [useStreamViewer] ICE Gathering 상태 변경:',
            ...
            const iceStatePolling = setInterval(() => {
              try {
                ...
                if (currentIceState !== iceConnectionState) {
                  console.log(
                    '🔄 [useStreamViewer] ICE 상태 polling으로 감지:',
                    {
                  ...
                  if (
                    ...
                    console.log(
                      '🎯 [useStreamViewer] ICE 연결 성공 (polling 감지)',
                    );
                  ...
                  // ICE 연결 실패 시 처리
                    ...
                    console.error(
                      '❌ [useStreamViewer] ICE 연결 실패 (polling 감지):',
                      currentIceState,
                    );
                ...
                if (
                  ...
                  setTimeout(() => {
                    if (rtcPeerConnection.iceConnectionState === 'new') {
                      console.error(
                        '❌ [useStreamViewer] ICE 연결이 30초 동안 new 상태 - 연결 실패',
                      );
                      setConnectionStatus('error');
                      setError({
                        code: 'ICE_CONNECTION_STUCK',
                        message:
                          'WebRTC 연결을 시작할 수 없습니다. 네트워크 설정을 확인해 주세요.',
                      });
                      clearInterval(iceStatePolling);
                    }
                  }, 30000);
                ...
                console.error(
                  '❌ [useStreamViewer] ICE polling 중 오류:',
                  error,
                );
                clearInterval(iceStatePolling);
              }
        ...
        const setupSessionEventListeners = (
            ...
            try {
              ...
              const handleStreamPlaying = () => {
                ...
                try {
                  ...
                  console.warn(
                    '⚠️ [useStreamViewer] RTCPeerConnection 접근 실패, 기본값 사용',
                  );
                }
                ...
                if (
                  ...
                  console.log(
                    '⚡ [useStreamViewer] ICE 연결 이미 완료 - 즉시 성공 처리',
                  );
                  setConnectionStatus('connected');
                  toast.success('스트림에 연결되었습니다.');
                  onStreamConnectedRef.current?.();
                ...
                if (
                  (hasVideoTracks || hasAudioTracks) &&
                  (iceConnectionState === 'connected' ||
                    iceConnectionState === 'completed')
                ) {
                  console.log(
                    '✅ [useStreamViewer] 실제 스트림 연결 완료 (ICE + 미디어)',
                  );
                  setConnectionStatus('connected');
                  toast.success('스트림에 연결되었습니다.');
                  onStreamConnectedRef.current?.();
                  ...
                  if (
                    ...
                    if (iceConnectionState === 'new') {
                      setTimeout(() => {
                        console.warn(
                          '🔄 [useStreamViewer] ICE 연결이 new 상태에서 멈춤 - 연결 분석 시도',
                        );
                        ...
                        try {
                          ...
                          if (rtcConnection) {
                            console.log(
                              '🔍 [useStreamViewer] RTCPeerConnection 상태:',
                              {
                                iceConnectionState:
                            ...
                            if (rtcConnection.iceConnectionState === 'new') {
                              console.warn(
                                '⚠️ [useStreamViewer] ICE 연결이 시작되지 않음 - 수동 재협상 필요',
                              );
                              ...
                              setError({
                                ...
                                message:
                                  'WebRTC 연결이 시작되지 않았습니다. 페이지를 새로고침해 주세요.',
                              });
                            }
                          } else {
                            console.warn(
                              '❌ [useStreamViewer] RTCPeerConnection을 찾을 수 없음',
                            );
                          }
                        } catch (error) {
                          console.error(
                            '❌ [useStreamViewer] ICE 상태 확인 중 오류:',
                            error,
                          );
                        }
                      }, 5000);
              ...
              playingTimeout = setTimeout(() => {
                console.warn(
                  `⏱️ [useStreamViewer] streamPlaying 타임아웃 (${timeoutDuration}ms)`,
                );
                console.log(
                  '🔍 [useStreamViewer] ICE 연결 상태:',
                  iceConnectionState,
                );
                ...
                if (
                  ...
                  console.log(
                    '✅ [useStreamViewer] ICE 연결 성공으로 스트림 연결 완료 처리',
                  );
                  setConnectionStatus('connected');
                  onStreamConnectedRef.current?.();
                } else {
                  console.warn(
                    `❌ [useStreamViewer] ICE 연결 실패 (상태: ${iceConnectionState}) - 연결 불완전`,
                  );
          ...
          session.on('exception', exception => {
            ...
            let errorMessage =
              exception.message || '스트림 연결 중 오류가 발생했습니다.';
            let shouldRetry = false;

            // 네트워크 품질과 에러 타입에 따른 개선된 에러 처리
            if (
              exception.message?.includes('401') ||
              exception.message?.includes('not valid')
            ) {
              errorMessage =
                '사장님이 아직 방송을 시작하지 않았습니다. 잠시 후 다시 시도해주세요.';
            } else if (exception.name === 'ICE_CONNECTION_DISCONNECTED') {
              // ICE 연결 끊김 - 브라우저 WebRTC의 정상적인 재연결 과정
              console.info(
                '🔌 [useStreamViewer] 브라우저 ICE 연결 재설정 중... (정상 과정)',
              );
              ...
              console.info(
                '⏳ [useStreamViewer] 브라우저에서 스트림 재생 준비 중... (정상 과정)',
              );
              ...
              exception.message?.includes('Request has timed out')
            ) {
        ...
        while (connectionAttempts <= maxRetries) {
          try {
            console.log(
              `🔄 [useStreamViewer] OpenVidu 연결 시도 ${connectionAttempts + 1}/${maxRetries + 1}`,
            );
            ...
            const connectWithTimeout = new Promise<void>((resolve, reject) => {
              ...
              const connectTimeout = setTimeout(() => {
                reject(
                  new Error(
                    `세션 연결 시간 초과 (${timeoutDuration}ms, 네트워크: ${networkQuality})`,
                  ),
                );
              }, timeoutDuration);
            ...
            if (connectionAttempts > maxRetries) {
              ...
              const errorMessage =
                connectionError instanceof Error
                  ...
                  : '알 수 없는 오류';
              throw new Error(
                `세션 연결 실패 (${connectionAttempts}/${maxRetries + 1} 시도): ${errorMessage}`,
              );
            } else {
              // 재시도 대기 (더 긴 시간으로 증가)
              const retryDelay =
        ...
        const errorData: StreamPlayerError = {
          ...
          message:
            err instanceof Error ? err.message : '스트림 연결에 실패했습니다.',
        };
