...
const InventoryPredictionChart = ({ predictions }: InventoryPredictionChartProps) => {
  // null 체크 및 데이터 변환
  ...
  const getInventoryRisk = () => {
    ...
    const avgRatio = chartData.reduce((sum, item) => sum + item.inventoryRatio, 0) / chartData.length;
    if (avgRatio < 20) {
  ...
  const CustomTooltip = ({ active, payload, label }: {
    active?: boolean;
    payload?: Array<{ payload: { dailyQuantity: number; remainingQuantity: number; inventoryRatio: number; confidence: number } }>;
    label?: string;
  }) => {
      ...
      <CardContent>
        {predictions && chartData.length > 0 ? (
            ...
            <div className="mt-4 space-y-3">
              ...
              {riskLevel && (
                <div className="flex items-center justify-between">
                  ...
                  <Badge variant={riskLevel.color as "default" | "destructive" | "outline" | "secondary"}>
                    {riskLevel.text}
                  </Badge>
                </div>
              )}
              ...
              <div className="flex items-center justify-between">
                ...
                <span className="text-lg font-bold text-green-600">
                  {(chartData.reduce((sum, item) => sum + item.inventoryRatio, 0) / chartData.length).toFixed(1)}%
                </span>
              </div>
          ...
          <div className="flex h-80 items-center justify-center text-muted-foreground">
            {predictions === null ? '재고 예측 서비스를 사용할 수 없습니다' : '재고 예측 데이터가 없습니다'}
          </div>
        )}
      </CardContent>
    </Card>
