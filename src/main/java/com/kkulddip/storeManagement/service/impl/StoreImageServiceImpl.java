...
public class StoreImageServiceImpl implements StoreImageService {
    ...
    private StoreImageResponseDto processImageUpload(MultipartFile file, Long storeId, int order, List<String> uploadedFileNames) {
        try {
            ...
            throw new BusinessException(ErrorCode.STORE_IMAGE_UPLOAD_FAILED, "이미지 처리에 실패했습니다.");
        }
    }
    ...
    public void deleteImages(Long storeId, List<Long> deleteImageIds) {
        try {
            ...
            throw new BusinessException(ErrorCode.STORE_IMAGE_DELETE_FAILED, "이미지 삭제에 실패했습니다.");
        }
    }
    ...
    public void deleteImagesBeforeDeleteStore(Long storeId) {
        ...
        try {
            ...
            throw new BusinessException(ErrorCode.STORE_IMAGE_DELETE_FAILED, "가게 이미지 삭제에 실패하여 가게를 삭제할 수 없습니다.");
        }
    }
    ...
    private void validateAddImageRequest(Long storeId, List<MultipartFile> images) {
        if (storeId == null || storeId <= 0) {
            throw new BusinessException(ErrorCode.STORE_MANAGEMENT_NOT_FOUND, "유효하지 않은 가게 ID입니다.");
        }

        // 가게 존재 확인
        if (!storeRepository.existsById(storeId)) {
            throw new BusinessException(ErrorCode.STORE_MANAGEMENT_NOT_FOUND, "가게를 찾을 수 없습니다.");
        }

        if (images == null || images.isEmpty()) {
            throw new BusinessException(ErrorCode.STORE_IMAGE_EMPTY_FILE, "업로드할 이미지가 없습니다.");
        }

        if (images.size() > MAX_IMAGES_PER_STORE) {
            throw new BusinessException(ErrorCode.STORE_IMAGE_LIMIT_EXCEEDED,
    ...
    private void validateDeleteImageRequest(Long storeId, List<Long> deleteImageIds) {
        ...
        if (deleteImageIds == null) {
            throw new BusinessException(ErrorCode.STORE_IMAGE_NOT_FOUND, "삭제할 이미지 ID 목록이 필요합니다.");
        }
        ...
        for (StoreImage image : images) {
            if (!image.getStoreId().equals(storeId)) {
                throw new BusinessException(ErrorCode.STORE_MANAGEMENT_NOT_OWNED, "삭제할 수 없는 이미지가 포함되어 있습니다.");
            }
        }
    }
