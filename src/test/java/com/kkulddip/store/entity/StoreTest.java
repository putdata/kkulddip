...
class StoreTest {
    ...
    void createStoreWithDefaultValues() {
        ...
        assertThat(store.getDdipBoxes()).isEmpty(); // 기본값 빈 리스트
        assertThat(store.getCreatedAt()).isNotNull(); // 기본값 현재 시간
    }

    @Test
    @DisplayName("Store 엔티티의 DdipBox 리스트를 관리할 수 있다")
    void manageDdipBoxes() {
        // given
        Store store = Store.builder()
                .storeId(1L)
                .storeName("테스트 마트")
        ...
        // when
        store.setDdipBoxes(Arrays.asList(ddipBox1, ddipBox2));

        // then
        assertThat(store.getDdipBoxes()).hasSize(2);
        assertThat(store.getDdipBoxes().get(0).getDdipboxName()).isEqualTo("유기농 세트");
        assertThat(store.getDdipBoxes().get(1).getDdipboxName()).isEqualTo("신선한 과일 세트");
        assertThat(store.getDdipBoxes().get(0).getStore()).isEqualTo(store);
        assertThat(store.getDdipBoxes().get(1).getStore()).isEqualTo(store);
    }
    ...
    void createStoreWithRequiredFieldsOnly() {
        ...
        assertThat(store.getDdipBoxes()).isEmpty();
    ...
    @Test
    @DisplayName("Store 엔티티의 빈 DdipBox 리스트도 정상적으로 처리한다")
    void handleEmptyDdipBoxes() {
        // given & when
        Store store = Store.builder()
                .storeName("빈 띱박스 테스트 마트")
                .ddipBoxes(Collections.emptyList())
                .build();

        // then
        assertThat(store.getDdipBoxes()).isEmpty();
        assertThat(store.getDdipBoxes()).isNotNull();
    }
